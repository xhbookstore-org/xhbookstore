...
@Tag(name = "认证接口", description = "微信手机号登录、Token刷新、登录态校验、退出登录")
@RestController
...
public class AuthController {
    ...
    @Operation(summary = "微信手机号登录",
        description = "通过微信授权code换取手机号，同时查member和sys_user表识别身份。新用户自动注册。入参: {\"code\":\"微信授权码\"}")
    ...
    @Operation(summary = "刷新Token", description = "用refreshToken换新accessToken。入参: {\"refreshToken\":\"...\"}")
    @PostMapping("/refresh-token")
    public ApiResponse<Map<String, Object>> refreshToken(@RequestBody Map<String, String> body) {
        String rt = body.get("refreshToken");
        if (StringUtils.isEmpty(rt)) throw new ApiException(ApiErrorCode.PARAM_INVALID, "缺少refreshToken");
        String secret = securityProperties.getJwt().getSecret();
        ...
        try {
            ...
            if (!"refresh".equals(c.get("type"))) throw new ApiException(ApiErrorCode.AUTH_TOKEN_INVALID, "无效的refreshToken");
            String at = Jwts.builder().setSubject(c.getSubject()).claim("isMember",c.get("isMember"))
                ...
                .signWith(SignatureAlgorithm.HS256,secret).compact();
            Map<String,Object> d=new HashMap<>(); d.put("accessToken",at); d.put("expiresIn",ae);
            return ApiResponse.success(d);
        } catch (ExpiredJwtException e) { throw new ApiException(ApiErrorCode.AUTH_TOKEN_EXPIRED, "refreshToken已过期"); }
        catch (Exception e) { throw new ApiException(ApiErrorCode.AUTH_TOKEN_INVALID, "无效的refreshToken"); }
    }

    @Operation(summary = "校验登录态", description = "校验accessToken有效性，返回isStaff/isMember/staffUserId/userId")
    @GetMapping("/session")
    public ApiResponse<Map<String, Object>> checkSession(HttpServletRequest request) {
        String uid = (String) request.getAttribute("userId");
        Map<String,Object> d=new HashMap<>(); d.put("valid",uid!=null);
        if (uid!=null) { d.put("isStaff",request.getAttribute("isStaff")); d.put("isMember",request.getAttribute("isMember"));
            d.put("staffUserId",request.getAttribute("staffUserId")); d.put("userId",uid); }
        return ApiResponse.success(d);
    }
    ...
    public ApiResponse<Map<String, Object>> logout() {
        Map<String,Object> d=new HashMap<>(); d.put("success",true); return ApiResponse.success(d);
    }

    private String maskPhone(String p) { return p!=null&&p.length()>6?p.substring(0,3)+"****"+p.substring(p.length()-4):p; }
}
